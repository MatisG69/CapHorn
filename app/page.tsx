...
export default function HomePage() {
  ...
  return (
    <div className="chc">
      ...
      {/* ─── SERVICES — liste éditoriale (ouvre la modale) ────── */}
      <section id="services" className="chc-section chc-section--white">
        <div className="chc-wrap">
          ...
          <div className="chc-svc">
            {SERVICES.map((s, i) => (
              <button key={s.n} type="button" className="chc-svc__row r" data-d={String((i % 3) + 1)} onClick={() => setOpen(s)} aria-label={`Détails — ${s.name}`}>
                <div className="chc-svc__num">{s.n}</div>
                <div>
                  <div className="chc-svc__name">{s.name}</div>
                  <p className="chc-svc__desc">{s.desc}</p>
                </div>
                <div className="chc-svc__tag">{s.tag}</div>
              </button>
            ))}
          </div>
        </div>
